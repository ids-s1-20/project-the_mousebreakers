...
astronauts %>%
  ...
  ggplot(aes(x = year_of_mission, fill = selection)) +
  geom_bar()
...
Include logistic regression, how previously mentioned variables contribute to gender, draw conclusions.
...
gender_fit <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(as.factor(sex) ~ total_number_of_missions + occupation, data = astronauts, family = "binomial")
...
- we've found that there is an upward trend in the number of women deployed in space missions. 
- Reasons/explanations for the trends we've been seeing - traditional gender roles - women in the house, men at work? Space being seen as dangerous? Men being seen as adventurers/pioneers, and not women?
- General trend: more women in space? Any predictions/projections for the future?

Are these varying factors affected by gender? Is there a progression towards gender equality in the space industry?
