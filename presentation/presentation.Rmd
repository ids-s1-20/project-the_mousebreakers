...
```{r}
#astronauts_nationality <- linear_reg() %>%
# set_engine("lm") %>%
 # fit(nationality ~ sex, data = astronauts)
#tidy(astronauts_nationality)
...
astronauts %>%
  ...
  filter(sex == "female") %>% 
  ggplot(aes(x = year_of_selection, fill = nationality)) +
  geom_bar()
...
#Nationality of Women in Space
 ...
 - Since 1959 people from over 40 different nationalities have ventured into outer-space, varying from U.K. and U.S. to Afghanistan and Mongolia. 
 - However, 
